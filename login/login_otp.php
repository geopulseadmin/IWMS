...
//                 $otp = isset($responseData['otp']) ? $responseData['otp'] : rand(1000, 9999);
//                 $otp_expiry_time = date('Y-m-d H:i:s', strtotime('+1 minute'));

//                 // Prepare the SQL statement to insert or update the OTP record
//                 $stmt = $pdo->prepare("
//                     INSERT INTO user_otp (contact_no, otp, otp_expiry, is_verified) 
//                     VALUES (:contact_no, :otp, :otp_expiry, FALSE)
//                     ON CONFLICT (contact_no)
//                     DO UPDATE SET 
//                         otp = EXCLUDED.otp,
...
//                 header("Location: verify_otp.php?contact_no=" . urlencode($contact_no));
//                 exit();
//             } else {
    ...
    if (preg_match('/^[0-9]{10}$/', $raw_contact_no)) {
        ...
        try {
            ...
            if (isset($responseData['type']) && strtolower($responseData['type']) === 'success') {
                ...
                header("Location: verify_otp.php?contact_no=" . urlencode($contact_no) . "&Work_ID=" . urlencode($Work_ID));
                exit();
            } else {
                // Handle failure response from MSG91
                $error_message = isset($responseData['message']) ? $responseData['message'] : 'Unknown error';
